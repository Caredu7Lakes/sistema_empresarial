...
/**
 * Adaptador do canal Redes Sociais.
 * Produção: API nativa da rede (sem custo de envio; sujeito a rate limit).
 */
public class ServicoRedesSociais implements ServicoMensagem {
    ...
    public ResultadoEnvio enviar(Mensagem mensagem) {
        ...
        String idProvedor = "RedesSociais-" + Integer.toHexString(mensagem.chaveIdempotencia().hashCode());
        System.out.printf("[%s] -> %s | %s%n", canal(), mensagem.destinatario(), mensagem.conteudo());
        return ResultadoEnvio.aceito(canal(), idProvedor);
    }

    @Override public String canal() { return "Redes Sociais"; }
}
